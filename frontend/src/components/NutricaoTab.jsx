  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
        ...
        <span style={{ fontSize: 11, color:'var(--text-muted)' }}>{current}g / {meta ?? '—'}g</span>
      </div>
...
function RefeicaoCard({ ref: r, planoId, alunoId, onDelete }) {
  ...
  return (
    <div style={{ background:'var(--bg-card)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 14, overflow: 'hidden' }}>
      <div
        onClick={() => setOpen(o => !o)}
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            <p style={{ fontSize: 14, fontWeight: 600, color:'var(--text-primary)' }}>{r.nome}</p>
            {r.horario && <p style={{ fontSize: 11, color:'var(--text-muted)' }}>{r.horario}</p>}
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ textAlign: 'right' }}>
            <p style={{ fontSize: 13, fontWeight: 600, color: '#fbbf24' }}>{kcal} kcal</p>
            <p style={{ fontSize: 11, color:'var(--text-muted)' }}>{prot.toFixed(1)}g prot.</p>
          </div>
          {open ? <ChevronUp style={{ width: 15, height: 15, color:'var(--text-muted)' }} /> : <ChevronDown style={{ width: 15, height: 15, color:'var(--text-muted)' }} />}
        </div>
      </div>
      {open && (
        <div style={{ padding: '0 16px 14px', borderTop: '1px solid rgba(255,255,255,0.04)' }}>
          {r.alimentos.length === 0 ? (
            <p style={{ fontSize: 12, color:'var(--text-muted)', padding: '10px 0' }}>Nenhum alimento adicionado</p>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: 10 }}>
              <thead>
                <tr>
                  {['Alimento', 'Qtd', 'Kcal', 'Prot', 'Carbo', 'Gord'].map(h => (
                    <th key={h} style={{ fontSize: 10, fontWeight: 600, color:'var(--text-muted)', textAlign: 'left', padding: '4px 8px 4px 0', textTransform: 'uppercase', letterSpacing: '0.04em' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {r.alimentos.map((a, i) => (
                  <tr key={i} style={{ borderTop: '1px solid rgba(255,255,255,0.03)' }}>
                    <td style={{ padding: '6px 8px 6px 0', fontSize: 13, color:'var(--text-secondary)', fontWeight: 600 }}>{a.nome}</td>
                    <td style={{ padding: '6px 8px 6px 0', fontSize: 12, color:'var(--text-muted)' }}>{a.qtd}</td>
                    <td style={{ padding: '6px 8px 6px 0', fontSize: 12, color: '#fbbf24', fontWeight: 600 }}>{a.kcal ?? '—'}</td>
                    <td style={{ padding: '6px 8px 6px 0', fontSize: 12, color:'var(--text-muted)' }}>{a.prot != null ? `${a.prot}g` : '—'}</td>
                    <td style={{ padding: '6px 8px 6px 0', fontSize: 12, color:'var(--text-muted)' }}>{a.carbo != null ? `${a.carbo}g` : '—'}</td>
                    <td style={{ padding: '6px 8px 6px 0', fontSize: 12, color:'var(--text-muted)' }}>{a.gord != null ? `${a.gord}g` : '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
            <button onClick={() => onDelete(r.id)} style={{ background: 'rgba(248,113,113,0.1)', border: '1px solid rgba(248,113,113,0.2)', borderRadius: 10, color: '#FF8078', cursor: 'pointer', fontSize: 12, fontWeight: 600, padding: '6px 12px', display: 'flex', alignItems: 'center', gap: 5 }}>
...
export default function NutricaoTab({ alunoId }) {
  ...
  if (isLoading) return <div style={{ padding: 24, color:'var(--text-muted)' }}>Carregando…</div>

  if (!plano) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        <div style={{ background:'var(--bg-card)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 18, padding: '28px 24px', textAlign: 'center' }}>
          ...
          <button onClick={() => setShowNovoPlan(true)} style={{ background: '#E8342B', border: 'none', borderRadius: 13, color: 'white', cursor: 'pointer', fontWeight: 600, fontSize: 14, padding: '11px 24px' }}>
            Criar plano alimentar
          </button>
        </div>
        {showNovoPlan && (
          <div style={{ background:'var(--bg-card)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 18, padding: 24 }}>
            <form onSubmit={handleCriarPlano} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <h3 style={{ fontFamily: 'Inter, sans-serif', fontSize: 16, fontWeight: 600, color:'var(--text-primary)', marginBottom: 4 }}>Novo plano alimentar</h3>
              <input value={formPlan.nome} onChange={setFP('nome')} placeholder="Nome do plano" style={{ background:'var(--bg-card)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 11, color:'var(--text-primary)', fontSize: 14, padding: '10px 14px' }} />
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10 }}>
                {[['objetivo_kcal','Kcal/dia'],['objetivo_proteina','Proteína (g)'],['objetivo_carbo','Carbo (g)'],['objetivo_gordura','Gordura (g)']].map(([k, label]) => (
                  <div key={k}>
                    ...
                    <input type="number" value={formPlan[k]} onChange={setFP(k)} placeholder="0" style={{ width: '100%', background:'var(--bg-card)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, color:'var(--text-primary)', fontSize: 14, padding: '8px 10px', boxSizing: 'border-box' }} />
                  </div>
                ))}
              </div>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <button type="submit" disabled={criar.isPending} style={{ flex: 1, background: '#E8342B', border: 'none', borderRadius: 12, color: 'white', cursor: 'pointer', fontWeight: 600, fontSize: 14, padding: '11px' }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      {/* Header do plano */}
      <div style={{ background:'var(--bg-card)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 18, padding: '18px 20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          <div>
            <p style={{ fontFamily: 'Inter, sans-serif', fontSize: 16, fontWeight: 600, color:'var(--text-primary)' }}>{plano.nome}</p>
            {plano.observacoes && <p style={{ fontSize: 12, color:'var(--text-muted)', marginTop: 4 }}>{plano.observacoes}</p>}
          </div>
          <button onClick={() => { if (confirm('Remover plano?')) deletarPlano.mutate(plano.id) }} style={{ background: 'rgba(248,113,113,0.1)', border: '1px solid rgba(248,113,113,0.2)', borderRadius: 10, color: '#FF8078', cursor: 'pointer', fontSize: 12, fontWeight: 600, padding: '6px 12px' }}>
            Remover plano
          </button>
        </div>
        {/* Macros */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
              <span style={{ fontSize: 11, fontWeight: 600, color:'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Calorias</span>
              <span style={{ fontSize: 11, color: '#fbbf24', fontWeight: 600 }}>{totalKcal} / {plano.objetivo_kcal ?? '—'} kcal</span>
            </div>
          ...
          <MacroBar label="Proteína" current={Math.round(totalProt)} meta={plano.objetivo_proteina} color="#FF8078" />
      ...
      {!showNovaRef ? (
        <button onClick={() => setShowNovaRef(true)} style={{ background: 'rgba(52,211,153,0.08)', border: '1px dashed rgba(52,211,153,0.25)', borderRadius: 14, color: '#34d399', cursor: 'pointer', fontWeight: 600, fontSize: 14, padding: '13px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
          <Plus style={{ width: 16, height: 16 }} /> Adicionar refeição
        </button>
      ) : (
        <div style={{ background:'var(--bg-card)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 18, padding: 20 }}>
          <form onSubmit={handleAddRef} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <h4 style={{ fontFamily: 'Inter, sans-serif', fontSize: 15, fontWeight: 600, color:'var(--text-primary)' }}>Nova refeição</h4>
            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 10 }}>
              <div>
                <label style={{ fontSize: 11, fontWeight: 600, color:'var(--text-muted)', display: 'block', marginBottom: 5 }}>Nome da refeição</label>
                <select value={nomeRef} onChange={e => setNomeRef(e.target.value)} style={{ width: '100%', background:'var(--bg-card)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, color:'var(--text-primary)', fontSize: 14, padding: '9px 12px' }}>
                  <option value="">Selecione…</option>
                  {REFEICOES_PADRAO.map(r => <option key={r} value={r}>{r}</option>)}
                </select>
              </div>
              <div>
                <label style={{ fontSize: 11, fontWeight: 600, color:'var(--text-muted)', display: 'block', marginBottom: 5 }}>Horário</label>
                <input type="time" value={horarioRef} onChange={e => setHorarioRef(e.target.value)} style={{ width: '100%', background:'var(--bg-card)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, color:'var(--text-primary)', fontSize: 14, padding: '9px 12px', boxSizing: 'border-box' }} />
              </div>
            </div>
            <p style={{ fontSize: 12, fontWeight: 600, color:'var(--text-muted)', marginBottom: -6 }}>Alimentos</p>
            {alimentosRef.map((a, i) => (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 60px 60px 60px 60px 32px', gap: 6, alignItems: 'end' }}>
                {[['nome','Alimento'],['qtd','Qtd'],['kcal','Kcal'],['prot','Prot(g)'],['carbo','Carbo(g)'],['gord','Gord(g)']].map(([k, ph]) => (
                  <div key={k}>
                    {i === 0 && <label style={{ fontSize: 10, color:'var(--text-muted)', display: 'block', marginBottom: 4 }}>{ph}</label>}
                    <input value={a[k]} onChange={setAlim(i, k)} placeholder={ph} type={['kcal','prot','carbo','gord'].includes(k) ? 'number' : 'text'} style={{ width: '100%', background:'var(--bg-card)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 9, color:'var(--text-primary)', fontSize: 12, padding: '7px 8px', boxSizing: 'border-box' }} />
                  </div>
                ))}
            ...
            <button type="button" onClick={addAlim} style={{ background: 'none', border: 'none', color: '#FF8078', cursor: 'pointer', fontSize: 13, fontWeight: 600, textAlign: 'left', padding: 0 }}>+ Adicionar alimento</button>
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button type="button" onClick={() => setShowNovaRef(false)} style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, color:'var(--text-muted)', cursor: 'pointer', fontWeight: 600, fontSize: 14, padding: '11px 18px' }}>Cancelar</button>
            </div>
          </form>
        </div>
      )}
