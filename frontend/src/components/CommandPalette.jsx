  ...
  return (
    <>
      ...
      <button
        ...
        onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.07)'; e.currentTarget.style.color='var(--text-muted)' }}
      ...
      >
        ...
        <span>Buscar...</span>
        <kbd style={{ fontSize: 10, fontWeight: 600, padding: '1px 6px', borderRadius: 5, background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.1)', color:'var(--text-muted)', fontFamily: 'Inter, sans-serif' }}>
          ⌘K
        </kbd>
      </button>
          ...
          <>
            ...
            >
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 18px', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
                <Search style={{ width: 18, height: 18, color: '#E8342B', flexShrink: 0 }} aria-hidden="true" />
                <input
                  ref={inputRef}
                  value={query}
                  ...
                  style={{
                    ...
                    color:'var(--text-primary)', fontSize: 16, fontFamily: 'Inter, sans-serif',
                  }}
                />
                {query && (
                  <button onClick={() => setQuery('')} aria-label="Limpar busca" style={{ background: 'none', border: 'none', cursor: 'pointer', color:'var(--text-muted)', padding: 2 }}>
                    <X style={{ width: 14, height: 14 }} />
                  </button>
                )}
                <kbd onClick={closePalette} style={{ fontSize: 10, fontWeight: 600, padding: '2px 8px', borderRadius: 6, background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.1)', color:'var(--text-muted)', fontFamily: 'Inter, sans-serif', cursor: 'pointer' }}>
                  ESC
                </kbd>
              </div>
              ...
              <div ref={listRef} style={{ maxHeight: 420, overflowY: 'auto', padding: '8px 0' }} role="listbox">
                {allItems.length === 0 ? (
                  <div style={{ padding: '40px 20px', textAlign: 'center' }}>
                    <p style={{ color:'var(--text-muted)', fontSize: 14 }}>Nenhum resultado para "<strong style={{ color:'var(--text-muted)' }}>{query}</strong>"</p>
                  </div>
                ) : (
                  Object.entries(groups).map(([group, items]) => (
                    <div key={group}>
                      <p style={{ fontSize: 10, fontWeight: 600, color:'var(--text-disabled)', textTransform: 'uppercase', letterSpacing: '0.1em', padding: '8px 18px 4px', fontFamily: 'Inter, sans-serif' }}>
                        {group}
                      </p>
                      {items.map((item) => {
                        const idx = flatIdx++
                        ...
                        return (
                          <div
                            ...
                            style={{
                              ...
                              borderLeft: isActive ? '2px solid #E8342B' : '2px solid transparent',
                              transition: 'all 0.1s',
                            }}
                          >
                            <div style={{ width: 34, height: 34, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, background: isActive ? 'rgba(99,102,241,0.2)' : 'rgba(255,255,255,0.05)' }}>
                              <item.icon style={{ width: 15, height: 15, color: isActive ? '#FF8078' : 'var(--text-muted)' }} aria-hidden="true" />
                            </div>
                            ...
                            {isActive && <ArrowRight style={{ width: 14, height: 14, color: '#E8342B', flexShrink: 0 }} aria-hidden="true" />}
                          </div>
                        )
                      })}
                    </div>
              ...
              <div style={{ padding: '10px 18px', borderTop: '1px solid rgba(255,255,255,0.05)', display: 'flex', gap: 16, alignItems: 'center' }}>
                {[
                  ...
                  <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                    {keys.map(k => (
                      <kbd key={k} style={{ fontSize: 10, padding: '1px 5px', borderRadius: 4, background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.1)', color:'var(--text-muted)', fontFamily: 'Inter, sans-serif' }}>{k}</kbd>
                    ))}
                    <span style={{ fontSize: 11, color:'var(--text-disabled)' }}>{label}</span>
                  </div>
                ))}
              </div>
            </motion.div>
