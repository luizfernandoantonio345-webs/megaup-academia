    ...
    <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, overflow: 'hidden' }}>
      <div onClick={() => setOpen(o => !o)} style={{ padding: '14px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <span style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 15, fontWeight: 800, color: '#fbbf24' }}>{kcal} kcal</span>
          {open ? <ChevronUp style={{ width: 14, height: 14, color: '#4B5768' }} /> : <ChevronDown style={{ width: 14, height: 14, color: '#4B5768' }} />}
        </div>
      </div>
        ...
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.05)', padding: '12px 16px' }}>
          {r.alimentos.map((a, i) => (
            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderBottom: i < r.alimentos.length - 1 ? '1px solid rgba(255,255,255,0.03)' : 'none' }}>
              <div>
                <p style={{ fontSize: 13, fontWeight: 600, color: '#94A3B8' }}>{a.nome}</p>
                <p style={{ fontSize: 11, color: '#3D4F6A' }}>{a.qtd}</p>
              </div>
              <div style={{ textAlign: 'right' }}>
                <p style={{ fontSize: 12, color: '#fbbf24', fontWeight: 700 }}>{a.kcal ?? '—'} kcal</p>
                <p style={{ fontSize: 11, color: '#4B5768' }}>P:{a.prot ?? '—'}g C:{a.carbo ?? '—'}g G:{a.gord ?? '—'}g</p>
              </div>
            </div>
          ))}
        </div>
...
export default function NutricaoAluno() {
  ...
  if (isLoading) return <div style={{ padding: 24, color: '#4B5768', textAlign: 'center' }}>Carregando…</div>

  if (!plano) {
    return (
      <div style={{ textAlign: 'center', padding: '60px 24px' }}>
        <div style={{ fontSize: 48, marginBottom: 16 }}>🥗</div>
        <p style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 18, fontWeight: 800, color: '#EFF6FF', marginBottom: 8 }}>Nenhum plano ainda</div>
        <p style={{ fontSize: 14, color: '#4B5768' }}>Seu personal ainda não criou um plano alimentar para você.</p>
      </div>
    )
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div>
        <h1 style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 22, fontWeight: 900, color: '#EFF6FF', letterSpacing: '-0.02em', marginBottom: 4 }}>{plano.nome}</h1>
        {plano.observacoes && <p style={{ fontSize: 13, color: '#4B5768' }}>{plano.observacoes}</p>}
      </div>
      ...
      <div style={{ background: 'linear-gradient(135deg,rgba(251,191,36,0.1),rgba(249,115,22,0.08))', border: '1px solid rgba(251,191,36,0.2)', borderRadius: 20, padding: '18px 20px' }}>
        <p style={{ fontSize: 11, fontWeight: 700, color: '#4B5768', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 12 }}>Total diário prescrito</p>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12 }}>
          {[
            ...
            <div key={m.label} style={{ textAlign: 'center' }}>
              <p style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 20, fontWeight: 900, color: m.color, lineHeight: 1 }}>{m.value}</p>
              <p style={{ fontSize: 10, color: '#3D4F6A', marginTop: 4, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em' }}>{m.label}</p>
              {m.meta && <p style={{ fontSize: 10, color: '#3D4F6A' }}>meta: {m.meta}</p>}
            </div>
          ))}
        </div>
      </div>
