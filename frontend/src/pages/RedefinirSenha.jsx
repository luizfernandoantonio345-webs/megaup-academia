    ...
    return (
      <div style={{ minHeight: '100vh', background: '#070B14', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
        <div style={{ textAlign: 'center', maxWidth: 340 }}>
          ...
          <h2 style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 20, fontWeight: 800, color: '#EFF6FF', marginBottom: 8 }}>Link inválido</h2>
          <p style={{ fontSize: 14, color: '#4B5768', marginBottom: 20 }}>O link de redefinição é inválido ou expirou.</p>
        ...
        <div style={{ background: '#0E1525', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 24, padding: '36px 32px', boxShadow: '0 24px 64px rgba(0,0,0,0.5)' }}>
          {ok ? (
            <div style={{ textAlign: 'center' }}>
              ...
              <h2 style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 22, fontWeight: 900, color: '#EFF6FF', marginBottom: 10 }}>Senha redefinida!</h2>
              <p style={{ fontSize: 14, color: '#4B5768', marginBottom: 20 }}>Sua senha foi alterada com sucesso. Redirecionando para o login…</p>
            </div>
          ) : (
            <>
              <div style={{ marginBottom: 28 }}>
                <h2 style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 24, fontWeight: 900, color: '#EFF6FF', letterSpacing: '-0.02em', marginBottom: 8 }}>Nova senha</h2>
                <p style={{ fontSize: 14, color: '#4B5768' }}>Escolha uma senha forte para sua conta.</p>
              </div>
              ...
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div>
                  <label style={{ fontSize: 12, fontWeight: 700, color: '#4B5768', display: 'block', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Nova senha</label>
                  <div style={{ position: 'relative' }}>
                    ...
                    <input
                      ...
                      required
                      autoFocus
                      style={{ width: '100%', background: '#141D30', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, color: '#EFF6FF', fontSize: 15, padding: '13px 44px 13px 42px', boxSizing: 'border-box', outline: 'none' }}
                    />
                    <button type="button" onClick={() => setShowPass(v => !v)} style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: '#3D4F6A' }}>
                ...
                <div>
                  <label style={{ fontSize: 12, fontWeight: 700, color: '#4B5768', display: 'block', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Confirmar senha</label>
                  <input
                    type={showPass ? 'text' : 'password'}
                    value={confirma}
                    ...
                    required
                    style={{ width: '100%', background: '#141D30', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, color: '#EFF6FF', fontSize: 15, padding: '13px 14px', boxSizing: 'border-box', outline: 'none' }}
                  />
                </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {regras.map(r => (
                      <div key={r.label} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        <span style={{ fontSize: 12, color: r.ok ? '#34d399' : '#3D4F6A' }}>{r.label}</span>
                      </div>
                    ))}
                  </div>
                )}
                ...
                <button
                  ...
                  style={{ background: 'linear-gradient(135deg,#4f46e5,#7c3aed)', border: 'none', borderRadius: 14, color: 'white', cursor: loading || !senhaValida ? 'not-allowed' : 'pointer', fontWeight: 800, fontSize: 15, padding: '14px', marginTop: 4, opacity: loading || !senhaValida ? 0.7 : 1, boxShadow: '0 0 24px rgba(99,102,241,0.35)' }}
                >
