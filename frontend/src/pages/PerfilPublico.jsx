...
export default function PerfilPublico() {
  ...
  if (isError || !data) {
    return (
      <div style={{ minHeight: '100vh', background:'var(--bg-page)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, textAlign: 'center' }}>
        <div>
          <div style={{ fontSize: 48, marginBottom: 16 }}>🔍</div>
          <h1 style={{ fontFamily: 'Inter, sans-serif', fontSize: 22, fontWeight: 600, color:'var(--text-primary)', marginBottom: 8 }}>Perfil não encontrado</h1>
          <p style={{ color:'var(--text-muted)', fontSize: 14 }}>Este link de perfil não existe ou foi removido.</p>
        </div>
      </div>
    )
  }
    ...
    <div style={{ minHeight: '100vh', background:'var(--bg-page)', color:'var(--text-primary)', fontFamily: 'Inter, sans-serif' }}>
      ...
      <section style={{ position: 'relative', overflow: 'hidden', padding: '64px 24px 48px', textAlign: 'center' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          {/* Avatar */}
          <div style={{ width: 100, height: 100, borderRadius: '50%', margin: '0 auto 20px', background: data.foto_url ? `url(${data.foto_url}) center/cover` : 'linear-gradient(135deg,#4f46e5,#7c3aed)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 36, fontWeight: 600, color: 'white',  animation: 'float 6s ease-in-out infinite' }}>
          ...
          {data.cref && (
            <p style={{ fontSize: 13, color:'var(--text-muted)', marginBottom: 8 }}>CREF: {data.cref}</p>
          )}

          <p style={{ fontSize: 14, color:'var(--text-muted)', marginBottom: 4, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5 }}>
            <MapPin style={{ width: 13, height: 13 }} /> {data.academia}
          </p>

          {data.bio && (
            <p style={{ fontSize: 15, color:'var(--text-secondary)', maxWidth: 500, margin: '20px auto 0', lineHeight: 1.7 }}>{data.bio}</p>
          )}
        </div>
      </section>
      ...
      <section style={{ padding: '0 24px 40px' }}>
        <div style={{ maxWidth: 600, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
          ...
          <div style={{ background:'var(--bg-card)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 20, padding: '20px 22px', textAlign: 'center' }}>
            ...
            <p style={{ fontFamily: 'Inter, sans-serif', fontSize: 32, fontWeight: 600, color:'var(--text-primary)', letterSpacing: '-0.02em', lineHeight: 1 }}>{data.stats.total_treinos}</p>
            <p style={{ fontSize: 12, color:'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em', marginTop: 6 }}>Treinos entregues</p>
          </div>
        </div>
      </section>
      ...
      {especialidades.length > 0 && (
        <section style={{ padding: '0 24px 40px' }}>
          <div style={{ maxWidth: 600, margin: '0 auto' }}>
            <h2 style={{ fontFamily: 'Inter, sans-serif', fontSize: 18, fontWeight: 600, color:'var(--text-primary)', marginBottom: 16, textAlign: 'center' }}>Especialidades</h2>
      ...
      <section style={{ padding: '0 24px 80px', textAlign: 'center' }}>
        <div style={{ maxWidth: 480, margin: '0 auto', background: 'linear-gradient(135deg,rgba(99,102,241,0.1),rgba(124,58,237,0.08))', border: '1px solid rgba(99,102,241,0.2)', borderRadius: 24, padding: '36px 28px' }}>
          <h2 style={{ fontFamily: 'Inter, sans-serif', fontSize: 22, fontWeight: 600, color:'var(--text-primary)', marginBottom: 10 }}>
          ...
          <p style={{ fontSize: 12, color:'var(--text-disabled)', marginTop: 14 }}>Sem cartão de crédito</p>
        </div>
      </section>

      <footer style={{ borderTop: '1px solid rgba(255,255,255,0.05)', padding: '20px 24px', textAlign: 'center' }}>
        <p style={{ fontSize: 12, color:'var(--text-disabled)' }}>Powered by <strong style={{ color: '#6366f1' }}>GymPro</strong> — plataforma para personal trainers</p>
      </footer>
    </div>
  )
}
