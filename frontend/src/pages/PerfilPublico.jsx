...
export default function PerfilPublico() {
  ...
  if (isError || !data) {
    return (
      <div style={{ minHeight: '100vh', background: '#0C0C0D', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, textAlign: 'center' }}>
        <div>
          <div style={{ fontSize: 48, marginBottom: 16 }}>🔍</div>
          <h1 style={{ fontFamily: 'Inter, sans-serif', fontSize: 22, fontWeight: 600, color: '#F4F4F5', marginBottom: 8 }}>Perfil não encontrado</h1>
          <p style={{ color: '#71717A', fontSize: 14 }}>Este link de perfil não existe ou foi removido.</p>
        </div>
      </div>
    )
  }
    ...
    <div style={{ minHeight: '100vh', background: '#0C0C0D', color: '#F4F4F5', fontFamily: 'Inter, sans-serif' }}>
      ...
      <section style={{ position: 'relative', overflow: 'hidden', padding: '64px 24px 48px', textAlign: 'center' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          ...
          {data.cref && (
            <p style={{ fontSize: 13, color: '#71717A', marginBottom: 8 }}>CREF: {data.cref}</p>
          )}

          <p style={{ fontSize: 14, color: '#71717A', marginBottom: 4, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5 }}>
            <MapPin style={{ width: 13, height: 13 }} /> {data.academia}
          </p>

          {data.bio && (
            <p style={{ fontSize: 15, color: '#A1A1AA', maxWidth: 500, margin: '20px auto 0', lineHeight: 1.7 }}>{data.bio}</p>
          )}
        </div>
      </section>
      ...
      <section style={{ padding: '0 24px 80px', textAlign: 'center' }}>
        <div style={{ maxWidth: 480, margin: '0 auto', background: 'linear-gradient(135deg,rgba(99,102,241,0.1),rgba(124,58,237,0.08))', border: '1px solid rgba(99,102,241,0.2)', borderRadius: 24, padding: '36px 28px' }}>
          <h2 style={{ fontFamily: 'Inter, sans-serif', fontSize: 22, fontWeight: 600, color: '#F4F4F5', marginBottom: 10 }}>
          ...
          <p style={{ fontSize: 12, color: '#52525B', marginTop: 14 }}>Sem cartão de crédito</p>
        </div>
      </section>

      <footer style={{ borderTop: '1px solid rgba(255,255,255,0.05)', padding: '20px 24px', textAlign: 'center' }}>
        <p style={{ fontSize: 12, color: '#52525B' }}>Powered by <strong style={{ color: '#6366f1' }}>GymPro</strong> — plataforma para personal trainers</p>
      </footer>
    </div>
  )
}
