...
const MILESTONES = [
  { qty: 1, reward: 'Badge exclusivo "Embaixador GymPro"', icon: '🏅' },
...
export default function Referral() {
  ...
  function shareLink() {
    if (!data?.referral_link) return
    if (navigator.share) {
      navigator.share({ title: 'GymPro', text: 'Use meu link para criar sua conta no GymPro!', url: data.referral_link })
    } else {
      copyLink()
    }
  }
    ...
    <div style={{ padding: 24, maxWidth: 700, margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
          ...
          <div>
            <h1 style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 22, fontWeight: 900, color: '#EFF6FF', letterSpacing: '-0.02em', marginBottom: 2 }}>Indique e Ganhe</h1>
            <p style={{ fontSize: 13, color: '#4B5768' }}>Convide outros personals e ganhe recompensas exclusivas</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 14, marginBottom: 28 }}>
        <div style={{ background: '#0E1525', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 18, padding: '20px 22px' }}>
          <p style={{ fontSize: 11, color: '#4B5768', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 10 }}>Seu código</p>
          <p style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 28, fontWeight: 900, color: '#fbbf24', letterSpacing: '0.06em' }}>
            {isLoading ? '——' : data?.referral_code}
          </p>
        </div>
        <div style={{ background: '#0E1525', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 18, padding: '20px 22px' }}>
          <p style={{ fontSize: 11, color: '#4B5768', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 10 }}>Indicações</p>
          <p style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 28, fontWeight: 900, color: '#34d399' }}>{indicados}</p>
        </div>
        <div style={{ background: '#0E1525', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 18, padding: '20px 22px' }}>
          <p style={{ fontSize: 11, color: '#4B5768', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 10 }}>Próxima meta</p>
          <p style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 28, fontWeight: 900, color: '#818cf8' }}>{nextMilestone.qty}</p>
        </div>
      </div>

      {/* Link box */}
      <div style={{ background: '#0E1525', border: '1px solid rgba(99,102,241,0.2)', borderRadius: 20, padding: '22px 24px', marginBottom: 28 }}>
        <p style={{ fontSize: 12, fontWeight: 700, color: '#4B5768', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 14 }}>Seu link de indicação</p>
        <div style={{ display: 'flex', gap: 10, alignItems: 'stretch', flexWrap: 'wrap' }}>
          <div style={{ flex: 1, background: '#070B14', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 12, padding: '12px 16px', minWidth: 200, overflowX: 'auto' }}>
            <span style={{ fontSize: 13, color: '#4B5768', fontFamily: 'monospace', whiteSpace: 'nowrap' }}>
      ...
      {indicados < nextMilestone.qty && (
        <div style={{ background: '#0E1525', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 18, padding: '20px 24px', marginBottom: 28 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            <p style={{ fontSize: 13, fontWeight: 700, color: '#94A3B8' }}>Progresso para próxima recompensa</p>
          ...
          <p style={{ fontSize: 12, color: '#4B5768', marginTop: 10 }}>
            Faltam <strong style={{ color: '#818cf8' }}>{nextMilestone.qty - indicados} indicação{nextMilestone.qty - indicados !== 1 ? 'ões' : ''}</strong> para {nextMilestone.icon} {nextMilestone.reward}
          </p>
        </div>
      )}

      {/* Milestones */}
      <div style={{ background: '#0E1525', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 20, overflow: 'hidden' }}>
        <div style={{ padding: '18px 24px 14px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
          <p style={{ fontSize: 14, fontWeight: 800, color: '#EFF6FF' }}>Recompensas disponíveis</p>
        </div>
        {MILESTONES.map((m, i) => {
          ...
          return (
            <div key={m.qty} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '16px 24px', borderBottom: i < MILESTONES.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none', opacity: unlocked ? 1 : 0.55 }}>
              ...
              <div style={{ flex: 1 }}>
                <p style={{ fontSize: 14, fontWeight: 700, color: '#EFF6FF' }}>{m.reward}</p>
                <p style={{ fontSize: 12, color: '#4B5768', marginTop: 2 }}>{m.qty} indicação{m.qty !== 1 ? 'ões' : ''} confirmada{m.qty !== 1 ? 's' : ''}</p>
              </div>
              {unlocked ? (
                <Check style={{ width: 18, height: 18, color: '#34d399', flexShrink: 0 }} />
              ) : (
                <div style={{ fontSize: 11, fontWeight: 700, color: '#3D4F6A', background: 'rgba(255,255,255,0.04)', borderRadius: 8, padding: '4px 10px', flexShrink: 0 }}>
      ...
      <div style={{ marginTop: 24, background: 'rgba(99,102,241,0.06)', border: '1px solid rgba(99,102,241,0.15)', borderRadius: 18, padding: '18px 22px' }}>
        ...
        {['Compartilhe seu link exclusivo com outros personal trainers.', 'Quando alguém criar uma conta usando seu link, contamos como uma indicação.', 'Ao atingir as metas, entre em contato para resgatar sua recompensa.'].map((t, i) => (
          <div key={i} style={{ display: 'flex', gap: 10, marginBottom: i < 2 ? 8 : 0 }}>
            <span style={{ fontSize: 11, fontWeight: 900, color: '#6366f1', minWidth: 18, paddingTop: 1 }}>0{i + 1}</span>
            <p style={{ fontSize: 13, color: '#4B5768', lineHeight: 1.5 }}>{t}</p>
          </div>
        ))}
      </div>
    </div>
