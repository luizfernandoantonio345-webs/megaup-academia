  ...
  return (
    <div style={{ padding: 24, maxWidth: 800, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div>
          <h1 style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 22, fontWeight: 900, color: '#EFF6FF', letterSpacing: '-0.02em' }}>Alunos Inativos</h1>
          <p style={{ fontSize: 13, color: '#4B5768' }}>Identifique e reconecte quem sumiu</p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
        {[3, 7, 14, 30].map(d => (
          <button
            key={d}
            onClick={() => setDias(d)}
            style={{ background: dias === d ? 'rgba(249,115,22,0.15)' : 'rgba(255,255,255,0.04)', border: `1px solid ${dias === d ? 'rgba(249,115,22,0.3)' : 'rgba(255,255,255,0.07)'}`, borderRadius: 12, color: dias === d ? '#f97316' : '#4B5768', cursor: 'pointer', fontWeight: 700, fontSize: 13, padding: '8px 18px', transition: 'all 0.15s' }}
          >
            +{d} dias
          </button>
        ))}
      ...
      {!isLoading && data && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 14 }}>
          <div style={{ background: '#0E1525', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 18, padding: '20px 22px' }}>
            <p style={{ fontSize: 11, fontWeight: 700, color: '#4B5768', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8 }}>Sem treinar há +{dias}d</p>
            <p style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 36, fontWeight: 900, color: '#f97316' }}>{data.total}</p>
          </div>
      ...
      {isLoading ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} style={{ background: '#0E1525', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 16, height: 80, animation: 'pulse 1.5s infinite' }} />
          ))}
        </div>
      ) : alunos.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '48px 24px', background: '#0E1525', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 20 }}>
          <div style={{ fontSize: 40, marginBottom: 12 }}>🎉</div>
          <p style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 18, fontWeight: 800, color: '#EFF6FF', marginBottom: 8 }}>Todos em dia!</p>
          <p style={{ fontSize: 14, color: '#4B5768' }}>Nenhum aluno inativo nos últimos {dias} dias.</p>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {alunos.map(a => (
            <div key={a.id} style={{ background: '#0E1525', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, padding: '16px 18px', display: 'flex', alignItems: 'center', gap: 14 }}>
              {/* Avatar */}
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ fontSize: 14, fontWeight: 700, color: '#EFF6FF', marginBottom: 3 }}>{a.nome}</p>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                    <Clock style={{ width: 12, height: 12, color: '#f97316' }} />
                    <span style={{ fontSize: 12, color: '#4B5768' }}>
              ...
              <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                ...
                <button
                  ...
                  style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 11, color: '#4B5768', cursor: 'pointer', padding: '7px 10px', display: 'flex', alignItems: 'center' }}
                >
